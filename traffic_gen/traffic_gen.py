...
def connection_matrix_line(flow: Flow, flow_id: int):
    ...
    return (
        ...
        f" {int(flow.start_time_s * 1e12)} size {flow.size_bytes}"
    )  # start time is in pico seconds. 

    """ return (
        f"{flow.src_idx}->{flow.dst_idx} id {flow_id} start"
        f" {int(0)} size {4500000}"
    ) """ 
...
def main():
    ...
    #avg_msg_size_bits = 8000000 * BYTE_TO_BIT
    avg_load_bps = bandwidth_bps * load
    avg_inter_arrival_time_ns = (avg_msg_size_bits / avg_load_bps) * NS_IN_S

    """ print("Avg Msg Size (bits):", avg_msg_size_bits)
    print("Avg Load (bps):", avg_load_bps)
    print("Avg Inter-arrival Time (ns):", avg_inter_arrival_time_ns) """
    # Generate flows.
    flow_list: list[Flow] = []
    for src_idc in range(nhost):
        count_per_host = 0
        flow_start_time_ns = base_time_ns 
        """ if count_per_host == 0:
            flow_start_time_ns = 0 """
        while flow_start_time_ns < base_time_ns + sim_duration_ns:
            ...
            flow_start_time_ns += int(
                #avg_inter_arrival_time_ns
                (flow_size_bytes * BYTE_TO_BIT * NS_IN_S / (bandwidth_bps)) / load
            )

            count_per_host = 0
            """ print(int(exponential_dist_sample(avg_inter_arrival_time_ns)))
            print(avg_inter_arrival_time_ns)
            print() """
