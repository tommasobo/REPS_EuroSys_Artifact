...
class Switch : public EventSource, public Drawable, public PacketSink {
 public:
    Switch(EventList& eventlist) : EventSource(eventlist, "none") { _name = "none"; _id = id++; _unique_id = unique_id++;};
    Switch(EventList& eventlist, string s) : EventSource(eventlist, s) { _name= s; _id = id++; _unique_id = unique_id++;}
