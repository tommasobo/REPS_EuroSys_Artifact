...
/* Use this constructor when there are multiple flows to one receiver
   ...
   NdpTunnelPullPacer */
NdpTunnelSink::NdpTunnelSink(NdpTunnelPullPacer* pacer) : DataReceiver("ndp_sink"),_cumulative_ack(0) , _total_received(0) 
{
    _src = 0;
    _pacer = pacer;
    _nodename = "ndpsink";
...
void NdpTunnelSink::receivePacket(Packet& pkt) {
    ...
    int size = p->size()-ACKSIZE_NDP; // TODO: the following code assumes all packets are the same size
...
{
    _packet_drain_time = (simtime_picosec)((Packet::data_packet_size()+ACKSIZE_NDP) * (pow(10.0,12.0) * 8) / linkspeed) / pull_rate_modifier;
