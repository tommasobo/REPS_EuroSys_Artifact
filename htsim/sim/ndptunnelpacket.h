...
public:
    ...
    inline static NdpTunnelPacket* newpkt(PacketFlow &flow, 
        ...
        p->set_attrs(flow, size+ACKSIZE_NDP, seqno+size-1); // The NDP sequence number is the first byte of the packet; I will ID the packet by its last byte.
        p->_type = NDP;
        p->_is_header = false;
        p->_bounced = false;
        p->_seqno = seqno;
    ...
    inline static NdpTunnelPacket* newpkt(PacketFlow &flow, const Route &route, 
                                          ...
                                          Packet* encap) {
        NdpTunnelPacket* p = _packetdb.allocPacket();
        p->set_route(flow,route,size+ACKSIZE_NDP,seqno+size-1); // The NDP sequence number is the first byte of the packet; I will ID the packet by its last byte.
        p->_type = NDP;
        p->_is_header = false;
        p->_bounced = false;
        p->_seqno = seqno;
    ...
    virtual inline void  strip_payload(uint16_t trim_size = ACKSIZE_NDP) {
