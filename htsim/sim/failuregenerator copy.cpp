...
void failuregenerator::read_failure_list() {
    ...
    string filename_full = "../failures_input/saved/saved_switch" + filename_input;
    ...
    //
    printf("Filename: %s\n", filename_input.c_str());
    string filename_full_cable = "../failures_input/saved/saved_cable" + filename_input;
     // Check if the file exists
...
void failuregenerator::save_failure_list() {
    ...
    if (use_list_cable_to_fail || use_list_switch_to_fail) {
        return;
    }
    

    printf("Len Switches and Cables to fail %d %d\n", _list_switch_to_fail.size(), _list_cables_to_fail.size());
    ...
    printf("Filename: %s\n", filename_input.c_str());
    std::ofstream outfile("../failures_input/saved/saved_switch" + filename_input);
    // Check if the file is opened successfully
    if (!outfile) {
        std::cerr << "Error opening file!" << std::endl;
        exit(0);
    ...
    std::ofstream outfile1("../failures_input/saved/saved_cable" + filename_input1);
...
bool failuregenerator::fail_new_cable(Pipe *p) {
    ...
    if (_list_cables_to_fail.size() > 0 && use_list_cable_to_fail && cable_id == _list_cables_to_fail[index_cable_to_fail]) {
        index_cable_to_fail++;
    } else if (_list_cables_to_fail.size() > 0 && use_list_cable_to_fail) {
        return false;
    }
