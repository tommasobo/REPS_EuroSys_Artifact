...
script_template = "./htsim_uec -sack_threshold 4000 -failed 4 -load_balancing_algo {algo} -tm {tm_file} {other_stuff} -end 90000 -seed 42 -paths {paths} {cc_strat} {trimming_strat} -topo {topo_file} -enable_qa_gate -linkspeed 400000 -ecn {kmin} {kmax} -q {queue} -cwnd {cwnd} > {output_dir}/{output_file}"
...
load_balancing_algos = ["reps", "ecmp", "flowlet"]
...
def getRunScript(dir_name, cc_algo, oversubscription, size, drop, tm_file, output_file, main_folder, lb_algo, num_entropy):
    ...
    if (oversubscription == 1):
        topo_file_name = f"topologies/reps/fat_tree_128_1os_{topo_tiers}t_400g.topo"
    ...
    command = script_template.format(other_stuff=other, queue=int(queue_size*1.0), kmin=ecn_min, kmax=ecn_max, cwnd=int(queue_size*1.5),  algo=lb_algo, paths=num_entropy, tm_file=tm_file, cc_strat=cc_algo, trimming_strat=drop, topo_file=topo_file_name, output_file=output_file, output_dir=dir_name)
...
def main():
    for msg_size in message_sizes:
        create_tm_files(msg_size, "permutation", topo_size)
        create_tm_files(msg_size, "tornado", topo_size)
        """ create_tm_files(msg_size, "permutation_bg", topo_size)
        create_tm_files(msg_size, "tornado_bg", topo_size) """
        create_tm_files(msg_size, "incast", topo_size, 32)
                    ...
                    for cc_algo in cc_algos:
                        for lb_algo in load_balancing_algos:
                            for num_entropy in num_entropies:
                                ...
                                command = getRunScript(dir_name, cc_algo, oversubscription, size_name, drop, file, output_file, args.output_folder, lb_algo, num_entropy)
