...
load_balancing_algos = ["oblivious", "plb", "mprdma", "bitmap", "freezing", "flowlet"]
...
failure_modes = ["5_percent_failed_switches_and_cables.txt", "5_percent_failed_switches.txt", "5_percent_failed_cables.txt", "fail_one_switch_one_cable.txt", "fail_one_cable.txt", "fail_one_switch.txt", "ber_cable_one_percent.txt", "ber_switch_one_percent.txt"]
...
def getRunScript(dir_name, trim_string, cc_algo, oversubscription, size, drop, tm_file, output_file, main_folder, lb_algo, num_entropy, failure_type):
    ...
    if (oversubscription == 1):
        topo_file_name = f"topologies/reps/fat_tree_{size}_1os_2t_400g.topo"
...
parser.add_argument('--cm', required=True, help='CM File', default="on")
parser.add_argument('--cache', required=False, help='Name for the experiment', default="")
parser.add_argument('--text', required=False, help='Name Title', default="")
                    ...
                    for failure_type in failure_modes:
                        for num_entropy in num_entropies:
                            for size in message_sizes:
                                ...
                                command = getRunScript(dir_name, trim_s, cc_algo, oversubscription, topology_size, drop, tm_file, output_file, args.output_folder, lb_algo, num_entropy, failure_type)
