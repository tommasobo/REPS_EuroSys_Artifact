...
script_template = "./htsim_uec {trim} {enable_bg_option} {failed_num} {asy_string} -sack_threshold 4000 -load_balancing_algo {algo} -tm {tm_file} {other_stuff} -end 90000 -seed 42 -paths {paths} {cc_strat} {trimming_strat} -topo {topo_file} -enable_qa_gate -linkspeed 400000 -ecn {kmin} {kmax} -q {queue} -cwnd {cwnd} > {output_dir}/{output_file}"
