...
# Function to create plots for specific flow size bins, supporting multiple files for comparison
def create_comparison_plots(filenames):
    # Parse all files and store them in a list
    dataframes = [parse_log_file(filename) for filename in filenames]
    
    # Define the custom bins: < 100KiB, 100KiB-1MiB, > 1MiB
    bin_edges = [0, 100 * 1024, 1024 * 1024, float('inf')]
    bin_labels = ['< 100KiB', '100KiB - 1MiB', '> 1MiB']

    # Prepare the figure with one extra column for overall statistics
    fig, axes = plt.subplots(2, 4, figsize=(24, 10))  # 2 rows, 3 bins + 1 column for overall stats

    # For each file, compute statistics and plot side-by-side for comparison
    for i, df in enumerate(dataframes):
        # Create bins based on flow size
        df['bin'] = pd.cut(df['flow_size'], bins=bin_edges, labels=bin_labels, include_lowest=True)
    ...
    # Save the plots
    plt.savefig("fcts_comparison.png", bbox_inches='tight', dpi=300)
    plt.savefig("fcts_comparison.pdf", bbox_inches='tight', dpi=300)

    plt.show()

# Main function to handle command-line arguments and run the script
def main():
    parser = argparse.ArgumentParser(description="Visualize Avg and 99th Percentile FCT for specific flow size bins across multiple files.")
    ...
    create_comparison_plots(args.filenames)
