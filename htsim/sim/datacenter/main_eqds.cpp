...
int main(int argc, char **argv) {
    ...
    while (i<argc) {
        if (!strcmp(argv[i],"-o")) {
            ...
            EqdsSrc::set_exp_avg_ecn(use_exp_avg_ecn);
            i++;
        } else if (!strcmp(argv[i], "-use_exp_avg_rtt")) {
            use_exp_avg_rtt = atoi(argv[i + 1]);
            printf("UseExpAvgRtt: %d\n", use_exp_avg_rtt);
            EqdsSrc::set_exp_avg_rtt(use_exp_avg_rtt);

        } else if (!strcmp(argv[i], "-use_reps")) {
            use_reps = atoi(argv[i + 1]);
            printf("UseREPS: %d\n", use_reps);
            EqdsSrc::set_reps(use_reps);
            i++;
    ...
    */
    ...
    for (size_t ix = 0; ix < no_of_nodes; ix++){
        pacers.push_back(new EqdsPullPacer(linkspeed, 0.99, EqdsSrc::_mtu, eventlist, ports));
        EqdsNIC* nic = new EqdsNIC(ix, eventlist, linkspeed, ports);
        nics.push_back(nic);
        if (log_nic) {
    ...
    for (size_t c = 0; c < all_conns->size(); c++){
        ...
        eqds_src = new EqdsSrc(traffic_logger, eventlist, *nics.at(src), ports);
        ...
        if (receiver_driven)
            eqds_snk = new EqdsSink(NULL,pacers[dest],*nics.at(dest), ports);
        else //each connection has its own pacer, so receiver driven mode does not kick in! 
            eqds_snk = new EqdsSink(NULL,linkspeed,0.99,EqdsSrc::_mtu,eventlist,*nics.at(dest), ports);
