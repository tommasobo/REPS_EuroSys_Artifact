        ...
        for (int i = 0; i < pipes_nlp_nup.size(); ++i) {
            for (int j = 0; j < pipes_nlp_nup[i].size(); ++j) {
                for (int k = 0; k < pipes_nlp_nup[i][j].size(); ++k) {
                    if ((rand() % 100) < (FAILURE_GENERATOR->cable_fail_max_percent*100)) {
                        if (stop_fail) {
                            continue;
                        }
                        FAILURE_GENERATOR->_list_cables_to_fail.push_back(pipes_nlp_nup[i][j][k]->getID());
                        printf("Cable to fail: %s (%d) - %s\n", pipes_nlp_nup[i][j][k]->nodename().c_str(), pipes_nlp_nup[i][j][k]->getID(), pipes_nlp_nup[i][j][k]->full_name.c_str());
