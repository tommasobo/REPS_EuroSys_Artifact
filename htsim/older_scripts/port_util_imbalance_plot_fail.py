...
pattern = r"LS0->US(\d+)"
# Input file path
file_pathreps = 'input_data_failures/repsportSwitch_LowerPod_{}_.txt'.format(ls_number)
file_pathobs = 'input_data_failures/obsportSwitch_LowerPod_{}_.txt'.format(ls_number)

queue_folder = 'input_data_failures/'
...
ax2.axhline(31*4096 / 1e3, color='gray', linestyle=':', linewidth=3)  # Convert to KB
ax2.axhline(120*4096 / 1e3, color='gray', linestyle=':', linewidth=3)  # Convert to KB

# Display grid
ax1.grid(True, which='both', linestyle=':', linewidth=0.75, alpha=0.75)


# Plot the barplot below the time-series plot
ax3 = plt.subplot(gs[1])
# Calculate overall utilization for each output port (no time buckets)
#ax3.set_xlim(0, 1350*1000)
...
queue_data = {}
for output_port in df['output_port'].unique():
    queue_file = os.path.join(queue_folder, f'repsqueueSizeLS{ls_number}->US{output_port}.txt')
    if os.path.exists(queue_file):
        queue_df = pd.read_csv(queue_file, header=None, names=['timestamp_ns', 'queue_size_bytes'])
        # Convert bytes to kilobytes
        queue_df['queue_size_bytes'] = queue_df['queue_size_bytes'] * LINK_SPEED_Gbps / 8 / 1e3
...
""" ax3.legend(unique_handles, unique_labels, loc='upper left')
 """
# Add horizontal dotted lines at specific values
ax4.axhline(31*4096 / 1e3, color='gray', linestyle=':', linewidth=3)  # Convert to KB
ax4.axhline(120*4096 / 1e3, color='gray', linestyle=':', linewidth=3)  # Convert to KB
...
# Display grid
ax3.grid(True, which='both', linestyle=':', linewidth=0.75, alpha=0.75)



plt.tight_layout()  # Adjust layout to prevent overlap
plt.savefig("combined_plot_fail_incremental.png", bbox_inches='tight')
plt.savefig("combined_plot_fail_incremental.pdf", bbox_inches='tight')
plt.show()
